// Temporary until https://github.com/porsager/postgres/pull/8 is merged
declare module 'postgres' {
   ...
   */
  function postgres<T extends PostgresTypeMap = {}>(
  ...
  namespace postgres {
    ...
    interface Options<T extends PostgresTypeMap>
    ...
    interface ArrayParameter<T extends Serializable[] = Serializable[]>
      extends Parameter<T> {
      array: true;
    }
    ...
    type Serializable = null | boolean | number | string | Date | object;
    ...
    type RowList<T extends readonly Row[]> = T &
      ResultMeta<T['length'], keyof T[number]>;
    ...
    interface Sql<TTypes extends PostgresTypeMap> {
       ...
       */
      <T extends object, U extends (keyof (T extends any[] ? T[number] : T))[]>(
        objOrArray: T,
        ...keys: U
      ): Helper<T, U>;
      ...
      array<T extends Serializable[] = Serializable[]>(
        value: T,
      ): ArrayParameter<T>;
      begin<T>(
      ...
      types: {
        [name in keyof TTypes]: <T extends TTypes[name]>(
          obj: T,
        ) => Parameter<T>;
      };
    ...
    interface TransactionSql<TTypes extends PostgresTypeMap>
      extends Sql<TTypes> {
      savepoint<T>(
        cb: (sql: TransactionSql<TTypes>) => T | Promise<T>,
      ): Promise<UnwrapPromiseArray<T>>;
